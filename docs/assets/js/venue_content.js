...
document.addEventListener("DOMContentLoaded", function () {
  getJSON(document.querySelector("meta[name=venueheader]").getAttribute("content"),
    function (header) {
      fetchJson(header);
      /* Last Update
      if (header.last_update) {
        document.getElementById("venue-update").textContent = "Last Update : " + header.last_update;
        fetchJson(header);
      } else {
        fetchJson(header);
      }
...
async function fetchJson(header) {
  const result = await fetch(header.data_url);
